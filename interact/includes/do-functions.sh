...
instances() {
	project="$(jq -r '.project' $AXIOM_PATH/axiom.json)"
	IFS=$'\n' resources=($(doctl projects resources list $project -o json | jq -r '.[].urn' | cut -d ':' -f3))
	filter="["
	for ((i=0; i<${#resources[@]}; i++)); do
		if [[ $((i+1)) == ${#resources[@]} ]]; then
			filter+="${resources[$i]}"
		else
			filter+="${resources[$i]},"
		fi
	...
	filter+="]"
	doctl compute droplet list -o json | jq --argjson filter "$filter" '.[] | select(.id as $ids | $filter | index($ids))' | jq --slurp '.'
}
...
instance_ip() {
	instances | jq -r --arg name $1 '.[] | select(.name == $name) | .networks.v4[].ip_address'
}
