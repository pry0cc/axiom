...
OS=$(lsb_release -i | awk '{ print $3 }')
   ...
   if [[ $OS == "Arch" ]] || [[ $OS == "ManjaroLinux" ]]; then
      ...
      wget -O /tmp/doctl.tar.gz https://github.com/digitalocean/doctl/releases/download/v1.65.0/doctl-1.65.0-linux-arm64.tar.gz && tar -xvzf /tmp/doctl.tar.gz && sudo mv doctl /usr/bin/doctl && rm /tmp/doctl.tar.gz
   fi
fi
